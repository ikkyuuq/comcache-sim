...
const useSimStore = create((set, get) => ({
  ...
  associativityOptions: [1, 2, 4, 8, 16, 32, 64, 128],
  bitAddressOptions: [8, 16, 32, 64],
  cacheSizeOptions: [8, 16, 32, 64, 128, 256, 512, 1024],
  blockSizeOptions: [1, 2, 4, 8, 16, 32, 64, 128],
  writePolicyOptions: ["WRITE_BACK", "WRITE_THROUGH"],

  memoryLatencyOptions: [10, 20, 50, 100, 200, 500, 1000],
  memoryBandwidthOptions: [1, 2, 4, 8, 16, 32, 64, 128],
  busLatencyOptions: [1, 2, 4, 8, 16, 32, 64, 128],

  writeBufferSizeOptions: [1, 2, 4, 8, 16, 32, 64, 128],
  writeBufferDrainRateOptions: [1, 2, 4, 8, 16, 32, 64, 128],
  ...
  destructuringAddress: (address) => {
    ...
    const binaryAddress = parseInt(address, 2)
      .toString(2)
      .padStart(get().cacheConfig.bitAddress, "0");
    const offset = binaryAddress.slice(-offsetSize);
    const index =
      indexSize > 0
        ? parseInt(binaryAddress.slice(tagSize, tagSize + indexSize), 2)
        : 0;
  ...
  directMappedSteps: [
    ...
    },
    {
      ...
      action: (state) => {
        ...
        if (cache) {
          ...
          } else if (cache.valid && cache.tag !== state.tag) {
    ...
    },
    {
      ...
      actionMessage: (state) =>
        state.cacheResult === "HIT"
          ? "⚡ Cache Hit: Fast access achieved in " +
            get().cacheConfig.hitTime +
            " cycle(s)."
          : "😮 Cache Miss: Data retrieval from memory will incur " +
            get().cacheConfig.missPenalty +
            " cycle penalty.",
      action: (state) => {
    ...
    },
    {
      ...
      actionMessage: (state) =>
        ...
        `Hit Rate: ${(get().performanceMetrics.hitRate * 100).toFixed(1)}% ` +
        `(Hits: ${get().countCacheResult.hit} / Total: ${get().countCacheResult.hit + get().countCacheResult.miss})`,
      action: (state) => {
  ...
  setAssociativeSteps: [
    {
      ...
      preMessage: (state) =>
        ...
        "• Set Index – Determines which set (of " +
        get().destructuringCache().numSets +
        ") stores the data.\n" +
    ...
    },
    {
      ...
      actionMessage: (state) =>
        state.cacheResult === "REPLACE"
          ? "♻️ Replacing LRU Entry: The least-recently-used slot will be replaced."
      ...
      action: (state) => {
        ...
        if (state.cacheResult === "HIT") {
          let newCaches = state.caches.map((cache) =>
            cache.index === state.index
              ? {
                  ...cache,
    ...
    },
    {
      ...
      actionMessage: (state) =>
        ...
        `Usage in Set ${state.index}: ` +
        `${state.caches[state.index].ways.filter((w) => w.valid).length} of ${get().cacheConfig.associativity} ways occupied.`,
      action: (state) => {
        set(() => ({
          cacheResult: "HIT",
          action: "WRITE",
  ...
  fullyAssociativeSteps: [
    ...
    },
    {
      ...
      action: (state) => {
        set(() => ({ cacheResult: "HIT", action: "WRITE", actionToIndex: 0 }));
        set((prev) => ({
          caches: prev.caches.map((cacheSet) => ({
            ...cacheSet,
            ways: cacheSet.ways.map((way, i) => {
